...
export async function getFinalAIAnalysis(scores: Scores, hobbies: string): Promise<string> {
    const scoreArray = (Object.keys(scores) as Array<keyof Scores>)
      .map(typeId => ({ id: typeId, type: typeMap[typeId], score: scores[typeId] }));
    ...
    const prompt = `
        Você é 'Tryla', uma conselheira de carreira especialista e mentora para jovens brasileiros de 16 a 24 anos. Sua linguagem é inspiradora, empática e direta, como um irmão mais velho gente boa. Use emojis para deixar a conversa mais leve. Evite jargões corporativos.
        ...
        Sua tarefa é analisar a combinação ÚNICA desses dois pontos e gerar um resultado final. Não use resultados prontos. A resposta deve ser original e conectar o perfil técnico (RIASEC) com o pessoal (hobbies).

        **Estruture a resposta em Markdown, exatamente assim:**

        ### ✨ Missão Cumprida, ${profileString}!

        Uau! Sua combinação de talentos e paixões é super poderosa. Você tem a [principal qualidade do primeiro tipo RIASEC], a [principal do segundo] e a [principal do terceiro]. Isso, misturado com seu interesse por [mencionar algo dos hobbies], te dá um superpoder único para [criar/resolver/ajudar] em [contexto].
    ...
    try {
        ...
        return "### 🤔 Ops, algo deu errado!\n\nNão consegui gerar seu resultado personalizado. Tente novamente mais tarde ou verifique sua conexão com a internet e se a chave da API está configurada corretamente no arquivo `.env`.";
    }
}