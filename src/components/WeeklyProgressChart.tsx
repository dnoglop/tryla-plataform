      ...
      {Array.from({ length: 7 }).map((_, index) => (
        ...
        >
          <div className="w-full h-full flex items-end">
            <div
              className="w-4/5 mx-auto rounded-lg bg-muted"
...
export const WeeklyProgressChart: React.FC<WeeklyProgressChartProps> = ({
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery<FormattedWeeklyData[], Error>({
    ...
    <div className="rounded-2xl bg-card p-4 sm:p-6 shadow-sm border">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-1.5 bg-primary/10 text-primary px-3 py-1.5 rounded-full text-sm font-bold">
          <Flame size={20} />
          <span>{streak || 0} dias de foco!</span>
        </div>
      </div>
