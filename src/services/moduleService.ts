...
// ... (todas as interfaces e funções CRUD de get/create/update/delete permanecem as mesmas) ...
...
export const getModules = async (): Promise<Module[]> => {
    try {
      const { data, error } = await supabase.from('modules').select('*').order('order_index', { ascending: true });
      if (error) throw error;
      return data || [];
    } catch (error) { console.error("Error fetching modules:", error); throw error; }
  };
  ...
  export const getModuleById = async (id: number): Promise<Module | null> => {
    try {
      ...
      if (error) throw error;
      return data;
    } catch (error) { console.error("Error fetching module by id:", error); throw error; }
  };
  ...
  export const getPhasesByModuleId = async (moduleId: number): Promise<Phase[]> => {
    try {
      const { data, error } = await supabase.from('phases').select('*').eq('module_id', moduleId).order('order_index', { ascending: true });
      if (error) throw error;
      return data || [];
    } catch (error) { console.error("Error fetching phases by module id:", error); throw error; }
  };
  ...
  export const getPhaseById = async (id: number): Promise<Phase | null> => {
    try {
      ...
      if (error) throw error;
      return data;
    } catch (error) { console.error("Error fetching phase by id:", error); throw error; }
  };
  ...
  export const getQuestionsByPhaseId = async (phaseId: number): Promise<Question[]> => {
    try {
      const { data: quiz, error: quizError } = await supabase.from('quizzes').select('id').eq('phase_id', phaseId).maybeSingle();
      if (quizError) throw quizError;
      if (!quiz) return [];
  
      const { data: questions, error } = await supabase.from('questions').select('*, tips_question').eq('quiz_id', quiz.id).order('order_index', { ascending: true });
      if (error) throw error;
...
export const getUserPhaseStatus = async (userId: string, phaseId: number): Promise<PhaseStatus> => {
  ...
  export const getModuleProgress = async (userId: string, moduleId: number): Promise<number> => {
      try {
        ...
        const { data, error } = await supabase.from('user_phases').select('status').eq('user_id', userId).in('phase_id', phaseIds);
        if (error) { throw error; }
    ...
    export const isModuleCompleted = async (userId: string, moduleId: number): Promise<boolean> => {
      try {
        ...
        const phaseIds = phases.map(phase => phase.id);
        const { data, error } = await supabase.from('user_phases').select('phase_id').eq('user_id', userId).eq('status', 'completed').in('phase_id', phaseIds);
        if (error) { throw error; }
...
export const completePhaseAndAwardXp = async (userId: string, phaseId: number, moduleId: number, isQuiz: boolean): Promise<{ xpFromPhase: number; xpFromModule: number }> => {
  ...
  // 1. Verifica se a fase já foi concluída ANTES de qualquer coisa
  ...
  const alreadyCompleted = phaseProgress?.status === 'completed';

  // Se já estava completa, não fazemos nada e retornamos 0 XP.
  if (alreadyCompleted) {
    return awardedXp;
  }

  // 2. Se não estava completa, marca como concluída.
  await supabase
    .from('user_phases')
    .upsert({
      user_id: userId,
  ...
  // 3. Concede XP da fase (se não for quiz)
  if (!isQuiz) {
    // Insere no histórico de XP. A constraint UNIQUE na tabela `xp_history`
    // (se existir) impedirá duplicatas. Se não houver, a lógica de checagem inicial já previne.
    ...
    // Se não houve erro (ou seja, a inserção foi bem-sucedida), atribui o XP.
    if (!xpError) {
      awardedXp.xpFromPhase = 5;
    }
  }

  // 4. Verifica se o módulo foi concluído AGORA, após a atualização.
  const allModulePhases = await getPhasesByModuleId(moduleId);
  const phaseIds = allModulePhases.map(p => p.id);

  const { data: completedPhases, error: countError } = await supabase
    .from('user_phases')
  ...
  if (countError) {
    console.error("Erro ao contar fases completas:", countError);
    return awardedXp; // Retorna o que temos até agora
  }
  
  const moduleIsNowComplete = completedPhases.length === allModulePhases.length;
  
  if (moduleIsNowComplete) {
    // Concede XP do módulo se ainda não foi concedido
    const { data: moduleLog } = await supabase.from('xp_history').select('id').eq('user_id', userId).eq('source', 'MODULE_COMPLETION').eq('source_id', String(moduleId)).maybeSingle();
    if (!moduleLog) {
