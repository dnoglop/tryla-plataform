...
const OnboardingPage = () => {
  ...
  const handleNext = () => {
    ...
    setDirection(1);
    if (currentStep < onboardingSteps.length) setCurrentStep((s) => s + 1);
  };
  ...
  const handleMultiSelectToggle = (
    field: "development_areas" | "interests",
    value: string,
  ) => {
    setProfileData((prev) => {
  ...
  const canContinue = useMemo(() => {
    switch (currentStep) {
      ...
      case 3:
        return (
          !!profileData.career_goal.trim() &&
          !!profileData.current_challenge.trim()
        );
      case 4:
        return (
          profileData.development_areas.length > 0 &&
  ...
  const trackMutation = useMutation({
    ...
    onError: (error) =>
      toast.error("Erro ao criar sua trilha.", { description: String(error) }),
  });
  ...
  const onCropComplete = useCallback(
    (_c: Area, pixels: Area) => setCroppedAreaPixels(pixels),
    [],
  );
            ...
            >
              {currentStep === 1 && (
                <div className="text-center flex flex-col justify-center items-center h-full">
                  ...
                  <p className="text-muted-foreground max-w-md">
                    Prepare-se para descobrir seus talentos, desenvolver novas
                    habilidades e construir o futuro que você sempre sonhou.
                  </p>
                </div>
              )}
              ...
              {currentStep === 2 && (
                <div className="space-y-6 max-w-md mx-auto">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    />
                    <div
                      className="relative w-24 h-24 rounded-full bg-muted border-2 border-primary/20 flex items-center justify-center overflow-hidden cursor-pointer"
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      onChange={handleInputChange}
                      placeholder="Fale um pouco sobre você, suas paixões e objetivos..."
                ...
                <div className="space-y-6 max-w-md mx-auto">
                  <div>
                    ...
                    <Textarea
                      ...
                      onChange={handleInputChange}
                      placeholder="Ex: Ser um líder em uma empresa de tecnologia, abrir meu próprio negócio de design..."
                      required
                      className="min-h-[100px]"
                    />
                  </div>
                  <div>
                    <Label
                      htmlFor="current_challenge"
                      className="text-sm font-semibold mb-2 block"
                    >
                      E qual o seu maior desafio hoje para chegar lá? *
                    </Label>
                    <Textarea
                      id="current_challenge"
                      name="current_challenge"
                      value={profileData.current_challenge}
                      onChange={handleInputChange}
                      placeholder="Ex: Não sei por onde começar, falta de confiança, dificuldade em me organizar..."
                      required
                      className="min-h-[100px]"
                    />
                  </div>
                  <div>
                    <Label
                      ...
                      className="text-sm font-semibold mb-2 block"
                    >
                      O que você ama fazer no seu tempo livre?
                    </Label>
                    <Textarea
                      id="hobbies"
                      name="hobbies"
                      ...
                      onChange={handleInputChange}
                      placeholder="Ex: Jogar, ler, praticar esportes..."
                      className="min-h-[100px]"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    <Label className="text-sm font-semibold mb-3 block">
                      Quais áreas você mais quer desenvolver? *
                    </Label>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {developmentAreas.map((area) => (
                        <button
                          ...
                          className={`p-3 rounded-xl border-2 text-left transition-all ${profileData.development_areas.includes(area.label) ? "border-primary bg-primary/10" : "border-border hover:border-primary/50"}`}
                        >
                          <div className="flex items-center gap-2">
                            <area.icon
                              className={`w-4 h-4 ${profileData.development_areas.includes(area.label) ? "text-primary" : "text-muted-foreground"}`}
                            />
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {interestsList.map((item) => (
                        <button
                          key={item.id}
                          onClick={() =>
                            handleMultiSelectToggle("interests", item.label)
                          }
                          className={`p-3 rounded-xl border-2 text-left transition-all ${profileData.interests.includes(item.label) ? "border-primary bg-primary/10" : "border-border hover:border-primary/50"}`}
                        >
                          <div className="flex items-center gap-2">
                            <item.icon
                              className={`w-4 h-4 ${profileData.interests.includes(item.label) ? "text-primary" : "text-muted-foreground"}`}
                            />
                            <span className="text-sm font-medium">
                              {item.label}
                            </span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select
                        name="learning_style"
                        onValueChange={(v) =>
                          handleSelectChange("learning_style", v)
                        }
                    ...
                    <div>
                      ...
                      <Select
                        name="weekly_time"
                        onValueChange={(v) =>
                          handleSelectChange("weekly_time", v)
                        }
              ...
              {currentStep === 5 && (
                <div className="text-center flex flex-col justify-center items-center h-full">
                  ...
                  <h1 className="text-3xl font-bold text-foreground mb-4">
                    Tudo pronto, {profileData.full_name.split(" ")[0]}!
                  </h1>
                  <p className="text-muted-foreground max-w-md">
                    Sua jornada personalizada está prestes a ser criada. Clique
                    no botão abaixo para que nossa IA prepare o caminho ideal
                    para o seu sucesso.
                  </p>
                </div>
              )}
        ...
        <DialogContent className="sm:max-w-lg bg-card">
          <DialogHeader>
            ...
            <DialogDescription className="text-center pt-2">
              Com base em suas respostas, preparei um caminho especial para
              você.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 px-2 max-h-[60vh] overflow-y-auto pr-4">
            <div className="prose prose-sm dark:prose-invert max-w-none prose-h3:text-foreground prose-h3:uppercase prose-h3:mb-2 prose-h3:mt-6 first-of-type:prose-h3:mt-2">
